...
public class ActivityIndicatorUtil: UIView {
   ...
   // MARK: - Singleton
    ...
    //
    public static let shared = ActivityIndicatorUtil(frame: CGRect.zero)
    ...
    @discardableResult
    public class func show(_ title: String, animated: Bool = true) -> ActivityIndicatorUtil {
        let spinner = ActivityIndicatorUtil.shared
        ...
        if spinner.superview == nil {
            ...
            UIView.animate(withDuration: ActivityIndicatorUtil.standardAnimationDuration, delay: 0.0, options: .curveEaseOut, animations: {
                ...
                NotificationCenter.default.addObserver(
                    spinner,
                    selector: #selector(ActivityIndicatorUtil.orientationChangedAction),
            ...
            show(delay: ActivityIndicatorUtil.standardAnimationDuration, title: title, animated: true)
        }
    ...
    /// - Returns: The instance of the spinner
    @discardableResult
    public class func show(duration: Double, title: String, animated: Bool = true, completion: (() -> ())? = nil) -> ActivityIndicatorUtil {
        let spinner = ActivityIndicatorUtil.show(title, animated: animated)
    ...
    public class func show(delay: Double, title: String, animated: Bool = true) {
        ...
        ActivityIndicatorUtil.shared.delay(delay, completion: {
            if let index = delayedTokens.firstIndex(of: token) {
                delayedTokens.remove(at: index)
                ActivityIndicatorUtil.show(title, animated: animated)
            }
        })
    }
    ...
    /// - Returns: The instance of the spinner
    @discardableResult
    public class func show(progress: Double, title: String) -> ActivityIndicatorUtil {
        let spinner = ActivityIndicatorUtil.show(title, animated: false)
    ...
    @objc func orientationChangedAction() {
        if let _ = ActivityIndicatorUtil.containerView() {
            ActivityIndicatorUtil.shared.setNeedsLayout()
        }
    }
    
    public func updateFrame() {
        if let containerView = ActivityIndicatorUtil.containerView() {
            ActivityIndicatorUtil.shared.frame = containerView.bounds
            containerView.bringSubviewToFront(ActivityIndicatorUtil.shared)
        }
    }
